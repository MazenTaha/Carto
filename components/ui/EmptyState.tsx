...
export function EmptyState({ icon = 'inventory_2', title, description, actionLabel, actionHref, action, className }: EmptyStateProps) {
  return (
    <div className={cn('rounded-3xl border border-dashed border-slate-200 bg-white p-8 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900', className)}>
      ...
      {description && <p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-slate-500 dark:text-slate-400">{description}</p>}
